...
public class PlayerCollision : MonoBehaviour {
    ...
    void FixedUpdate() {
        ...
        void CheckGround() {
            ...
            if (contact[1] == 1) {
                player.PlayerOnSlope = false;
            } else if (contact[1] == 0 && contact[0] == 1) {
                player.PlayerOnSlope = true;
            }
        }
    }
    ...
    public bool IsGrounded(string layerName = null) {
        ...
        if (layerName == "Platform") {
            raycast = Physics2D.Raycast(player.collider.bounds.center + new Vector3(0, player.collider.bounds.extents.y), Vector2.down, (2 * player.collider.bounds.extents.y) + 0.2f, player.platformLayerMask);

            return raycast.collider;
        } else if (layerName == "Slope") {
            raycast = Physics2D.Raycast(player.collider.bounds.center + new Vector3(0, player.collider.bounds.extents.y), Vector2.down, (2 * player.collider.bounds.extents.y) + 0.4f, player.slopeLayerMask);
    ...
    private void OnCollisionEnter2D(Collision2D collision) {
        if (CheckGround(collision) == "Platform") {
            if (player.PlayerOnSlope) { player.PlayerOnSlope = false; }
        ...
        if (collision.gameObject.layer == LayerMask.NameToLayer("Enemy") && !player.PlayerIsDead) {
            player.Dead(true);
        }
    }
    ...
    private void OnTriggerEnter2D(Collider2D collision) {
        ...
        if (collision.gameObject.tag == "Slime" && !player.PlayerIsDead) {
            rigidbody.velocity = Vector2.zero;
            rigidbody.gravityScale = 0.05f;
        } else if (collision.gameObject.tag == "SecretRoom") {
            player.PlayerInSecretRoom = true;

            if (!player.PlayerIsDead) {
                secretRoomAnim.SetBool("showSecretRoom", true);
            }
        } else if (collision.gameObject.tag == "GoldenMedal" && !player.PlayerIsDead) {
        ...
        } else if (collision.gameObject.tag == "Magnifier" && !player.PlayerIsDead) {
        ...
        } else if (collision.gameObject.name == "Crown" && !player.PlayerIsDead) {
            //Finish Game
            particles[0].Play();
            particles[1].Play();
    ...
    private void OnTriggerExit2D(Collider2D collision) {
        ...
        if (!IsGrounded() && collisionCount <= 0 && player.PlayerIsDead && (collision.gameObject.layer == LayerMask.NameToLayer("Slope") || collision.gameObject.layer == LayerMask.NameToLayer("Platform")) && !player.PlayerInSecretRoom) {
            player.Dead(false);
        }
        ...
        if (collision.gameObject.tag == "Slime") {
            ...
            player.PlayerInSecretRoom = false;
